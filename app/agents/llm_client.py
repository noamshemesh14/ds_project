...
- constraint_manager: Add constraints to user's schedule (permanent or one-time). Use this when the user wants to add a constraint (e.g., "I have training on Monday 18:00-20:00", "I work every Tuesday 14:00-16:00", "I have a meeting on Wednesday 10:00-11:00 this week"). Requires: title (constraint name, e.g., "אימון", "עבודה"), start_time (HH:MM format), end_time (HH:MM format), days (list of days 0-6) or day_of_week (single day 0-6). Optional: description, is_permanent (True for permanent/recurring constraint, False for one-time, default: False if not specified), week_start (YYYY-MM-DD format for one-time constraints, defaults to current week), is_hard (True for hard constraint, False for soft, default: True). If user doesn't specify "permanent" or "recurring", it's one-time. The system will check for conflicts with existing constraints (rejects) and existing schedule (warns but allows).
...
- Examples:
  * "I have training on Monday 18:00-20:00" → title="training" or "אימון", day_of_week=1 (Monday), start_time="18:00", end_time="20:00", is_permanent=False (default)
  * "I work every Tuesday 14:00-16:00" → title="work" or "עבודה", day_of_week=2 (Tuesday), start_time="14:00", end_time="16:00", is_permanent=True
  * "I have a meeting on Wednesday 10:00-11:00 this week" → title="meeting" or "מפגש", day_of_week=3 (Wednesday), start_time="10:00", end_time="11:00", is_permanent=False, week_start=current week
  * "I have training on Wednesday 15/02/2025 18:00-20:00" → title="training", day_of_week=3 (Wednesday), start_time="18:00", end_time="20:00", date="2025-02-15" (will be converted to week_start=Sunday of that week)
  * "יש לי אימון ביום רביעי 15/02/2025 בשעה 18:00-20:00" → title="אימון", day_of_week=3 (Wednesday), start_time="18:00", end_time="20:00", date="2025-02-15"
